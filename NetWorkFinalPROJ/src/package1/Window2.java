...
public class Window2 {
	...
	private void initialize() {
		...
		frameIcon = Toolkit.getDefaultToolkit().createImage(
	    		  this.getClass().getResource("frameIcon.png"));
		...
		button.addMouseListener(new MouseListener(){
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				try { 
					...
					r1.exec ("cmd.exe /c"+ "start C:\\EhentaiImage");
	...
	void DownloadIMG(String imgURL,String imgsName) throws IOException, InterruptedException
	{	
		URL url = null;
		...
		try {
			...
			EhentaiIMGDownloader ehen = new EhentaiIMGDownloader(con.getInputStream(),imgsName,folderDst);
		...
		void getIMG(List<String> imgs) throws IOException, InterruptedException
		{
			...
			for(int i=0;i<imgs.size();i++)
			{
				...
				while((str=buf.readLine())!=null)
				{
					...
					while(mat.find())
					{
						DownloadIMG(mat.group(1),mat.group(2));
					}
				}
			}
